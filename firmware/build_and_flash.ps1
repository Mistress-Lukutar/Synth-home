...
<#
.SYNOPSIS
    Build and flash the ZigbeeHUB firmware using ESP-IDF.
...
.PARAMETER Port
    COM port the board is connected to. Defaults to COM3 (USB Serial JTAG).
...
.NOTES
    Copied and adapted from the SensorHUB project (ESP32-S3) for the ZigbeeHUB
    ESP32-C6 (RISC-V) target.
#>
...
# Directories idf.py expects to find in PATH (ESP32-C6 is RISC-V).
$toolPaths = @(
    Join-Path $PythonEnvPath 'Scripts'
    Join-Path $IdfToolsPath 'tools\riscv32-esp-elf\esp-15.2.0_20251204\bin'
...
$exportLog = Join-Path $env:TEMP "zigbeehub_export_$PID.log"
...
# --- Build ---
$buildLog = Join-Path $env:TEMP "zigbeehub_build_$PID.log"

if ($Clean) {
    $cleanLog = Join-Path $env:TEMP "zigbeehub_clean_$PID.log"
    Invoke-IdfCommand -Label "Cleaning previous build" -LogFile $cleanLog -Arguments @('fullclean')
}

Invoke-IdfCommand -Label "Building ZigbeeHUB project" -LogFile $buildLog -Arguments @('build')
...
$flashLog = Join-Path $env:TEMP "zigbeehub_flash_$PID.log"
